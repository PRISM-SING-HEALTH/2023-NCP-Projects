...
if 'df' not in st.session_state:
    st.session_state.df = DataFrame(columns=["HPO_id", "Given Term", "Official Term"]).reset_index(drop=True)
if 'prev_df' not in st.session_state:
    st.session_state.prev_df = DataFrame(columns=["HPO_id", "Given Term", "Official Term"]).reset_index(drop=True) 
...
def data_upload(data_entity):
    df = DataFrame(data_entity, columns=["HPO_id", "Given Term", "Official Term"]).reset_index(drop=True)
    return df  
...
def generate_aggrid_final(df, cellRend1, cellRend2, cellRend3):
    ...
    gd.configure_pagination(enabled=True)
    gd.configure_default_column(editable=True, groupable=True)
    gd.configure_column(field='HPO_id', headerCheckboxSelection=True)
...
def api_call(term, results_shown):
    ...
    :param term: clinical term that user wants assocaited HPO code for
    :return: None
    """
    res = [] 
    try:
...
def highlight(doc):
    ...
    list_doc = list(doc)
    sorted_phenotypes = st.session_state.tag_result_phenotagger + st.session_state.tag_result_phenobert 
    for tag in sorted_phenotypes:
        if tag[-1] == 'orange':
...
def submit(doc):
    ...
    # user input
    text_phenobert = doc.strip().replace("\n", " ")
    ...
    st.session_state.tag_result_phenobert_text, st.session_state.tag_result_phenobert = annotate_text(text_phenobert, use_longest=overlap_phenobert)
...
with c1:
    st.markdown('<p style="text-decoration: orange dashed underline 3px;" class="big-font"><strong>Phenobert Results<strong></p>', unsafe_allow_html=True)
    # bug: first selection won't show on aggrid table although 'selected rows' shows it was selected. Fixed by selecting again or pre-selecting. 
    grid_table_1 = generate_aggrid(df_phenobert, pre_row=[0], key='phenobert')


with c3:
    st.markdown('<mark style="background-color: yellow'+';"  class="big-font"><strong>Phenotagger Results<strong></mark>', unsafe_allow_html=True)
    # bug: first selection won't show on aggrid table although 'selected rows' shows it was selected. Fixed by selecting again or pre-selecting. 
    grid_table_2 = generate_aggrid(df_phenotagger, pre_row=[0], key='phenotagger')
...
st.session_state.sel_rows = grid_table_1['selected_rows'] + grid_table_2['selected_rows'] + grid_table['selected_rows'] 
